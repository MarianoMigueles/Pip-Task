...
let langSelected = localStorage.getItem('settingsVoices') ? JSON.parse(localStorage.getItem('settingsVoices')).lang : null;
let voiceSelected = localStorage.getItem('settingsVoices') ? JSON.parse(localStorage.getItem('settingsVoices')).voice : null;

export async function readText(text) {
    await Speakit.readText(text, langSelected, voiceSelected);
}

export function changeVoice(lang, voice) {
    langSelected = lang;
    voiceSelected = voice;
    updateSettingsVoices({ lang: langSelected, voice: voiceSelected });
}
