import { formatDateString, togglePlay, readText, updateTask, deleteTask, playSound } from "./main.js";
...
function renderTaskListScreenHTML() {
    ...
    main.innerHTML = `
        ...
        <section class="container right center-elements can-modal">
    ...
    function taskNotFound(container, type) {
        ...
        container.innerHTML = `
            <div class="not-found center-elements">
                <p>No se encontraron tareas ${type}</p>
            </div>
        `;
    }
...
function openTaskForm(task) {
    ...
    taskForm.innerHTML = `
        <div id="btn-top-container" class="center-elements">
            <button id="btn-close-task-form" type="button" class="icon material-symbols-outlined">close</button>
            <button id="btn-play-task-form" type="button" data-taskState="${task.getAttribute('data-taskState')}" data-needToglePlay="true" class="play-btn icon material-symbols-outlined">
                ${task.getAttribute('data-taskState') === 'pendiente' ? 'play_circle' : 'check_circle'}
            </button>
        </div>
        ...
        <div class="task-form-footer center-elements">
            <div id="btn-left-container" class="icon-container center-elements">
            ...
            <div id="btn-right-container" class="icon-container center-elements">
    ...
    if (task.getAttribute('data-taskState') === 'completado') {
        document.getElementById('btn-right-container').classList.add('completed');
    }
...
function attachFormEventListeners(task, taskForm) {
    ...
    const btnPlayTaskForm = document.getElementById('btn-play-task-form');
    btnPlayTaskForm.addEventListener('click', async () => {
        togglePlay(btnPlayTaskForm);

        if(btnPlayTaskForm.classList.contains('active')) {
            await readText(document.getElementById('task-form-input').value);
        }
